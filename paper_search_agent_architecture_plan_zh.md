...
### 7.5 继续补强 Provider Runtime / Policy
...
3. 继续拆分共享逻辑，降低 `search_common.py` 的集中度
4. 继续收敛 `provider runtime/policy`，补齐 query policy、日志和观测
5. 继续增强 Quick 的 hybrid ranking
6. 继续增强 Deep 的硬规则过滤和更稳定的 per-source LLM judge
7. 完成 `POST /v1/search/fusion`
8. 完成 `POST /v1/resolve/fulltext`
9. 增加统一错误码和自动测试
10. 最后再推进前端、Django 集成和 skill
