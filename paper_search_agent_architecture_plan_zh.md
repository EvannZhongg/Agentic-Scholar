...
截至 2026-03-31，当前仓库已经不再只是设计稿，而是一个可运行的后端原型。
...
3. 继续收敛已经落地的 `deep`，优先补齐 `2-of-4 / 3-of-4` 放宽组合、更严格的 `c4` 证据要求、更细的 source-aware recall/query policy，以及更稳定的 criterion-level evidence
